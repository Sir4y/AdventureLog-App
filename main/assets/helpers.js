...
// Polyfill and bridge for Geolocation API
(function() {
    if (!window.WebToApk || !window.WebToApk.getNativeLocation) return;

    const originalGetCurrentPosition = (navigator.geolocation && navigator.geolocation.getCurrentPosition) ? navigator.geolocation.getCurrentPosition.bind(navigator.geolocation) : null;
    let callbackIdx = 0;
    const isHttp = window.location.protocol === 'http:' && window.location.hostname !== 'localhost' && window.location.hostname !== '127.0.0.1';

    function customGetCurrentPosition(success, error, options) {
        if (!isHttp && originalGetCurrentPosition) {
            originalGetCurrentPosition(success, function(err) {
                if (err && (err.code === 1 || err.code === 2 || err.code === 3)) {
                    fallbackToNativeLocation(success, error);
                } else if (error) {
                    fallbackToNativeLocation(success, error);
                }
            }, options);
    ...
    function fallbackToNativeLocation(success, error) {
        const successName = '__geo_success_' + (++callbackIdx);
        const errorName = '__geo_error_' + callbackIdx;
        ...
        window[errorName] = function(err) {
            delete window[successName];
            delete window[errorName];
            if (typeof error === 'function') error(err);
        };

        window.WebToApk.getNativeLocation(successName, errorName);
    }
    ...
    navigator.geolocation.getCurrentPosition = customGetCurrentPosition;
    navigator.geolocation.watchPosition = function(success, error, options) {
        customGetCurrentPosition(success, error, options);
